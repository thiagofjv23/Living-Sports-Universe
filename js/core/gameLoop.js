...
// Junta as peças de LÓGICA: gera entidades, pede ao módulo esportivo
// para simular e PUBLICA os fatos no EventBus. Ele NÃO grava na
// memória (isso é do ouvinteHistorico) e NÃO desenha nada (isso é do
// renderizador). Aqui não há DOM.
//
// O TEMPO (dia/ano) é responsabilidade do calendario.js; quem decide
// QUANDO cada competição joga é o agendador.js.
// =============================================================
...
const RODADAS_POR_TEMPORADA = 6; // rodadas de cada competição por temporada
let anoAtual = ANO_PRESENTE; // ano corrente (sincronizado pelo calendário)
...
// cada confronto no EventBus. Função PURA de simulação: NÃO avança o
// tempo (isso é do calendário) — apenas carimba o ano e a DATA real
// do jogo. Chamada pelo agendador quando há rodada no dia.
...
function simularRodadaCompeticao(competicao, organizacoes) {
  ...
  for (let i = 0; i + 1 < embaralhadas.length; i += 2) {
    ...
    resultado.ano = anoAtual; // ano do presente
    resultado.dataSimulada = dataParaISO(dataAtual); // DATA real do jogo (calendário)
    EventBus.emit("PARTIDA_EQUIPES_FINALIZADA", resultado);
  }
}
